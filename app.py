...
# PREDICTIONS using pkl_rfc_log_norm_scale_ggm.pkl ------------------------------------------------------
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
@app.route("/home_rfc_log_norm_scale")
def home_rfc_log_norm_scale():
    return render_template("home_rfc_log_norm_scale.html")

@app.route("/predict_rfc_log_norm_scale", methods = ["POST"])
def predict_rfc_log_norm_scale():
    # ======================================================================================
    # Input Example:
    # [start_date, end_date, present_days, access, discussion, navigate, problem, wiki]
    # ["02-12-2020", "03-12-2020", 2, 8, 0, 6, 0, 0]
    # ======================================================================================
    ...
    print(start_date)
    cols = ["present_days", "access", "discussion", "navigate", "problem", "wiki"]
    in_features = [int(request.form[col]) for col in cols]
    in_features = [duration_in_days] + in_features

    # Log transform
    in_features = [np.log(i + 1) for i in in_features]
    # Normalize
    in_features = preprocessing.normalize(pd.DataFrame(in_features))
    # Standard scaling
    scale = StandardScaler()
    scaled_values = scale.fit_transform(in_features).reshape(1, -1)
